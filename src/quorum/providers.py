...
def list_all_models_sync() -> dict[str, list[ModelInfo]]:
    ...
    for provider_key in settings.extra_provider_keys():
        cfg = settings.get_extra_provider_config(provider_key)
        if cfg:
            result[provider_key] = [
                ModelInfo(id=model_id, provider=provider_key, display_name=display_name)
                for model_id, display_name in cfg.models
            ]
...
def get_provider_for_model(model_id: str) -> str | None:
    ...
    # Extra providers: groq:..., deepseek:..., etc.
    from .extra_providers import resolve_extra_provider_model

    resolved = resolve_extra_provider_model(model_id)
    if resolved:
        provider_key, _ = resolved
        settings = get_settings()
        if settings.is_extra_provider(provider_key):
            return provider_key
