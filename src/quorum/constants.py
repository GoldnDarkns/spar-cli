...
def get_available_providers_fast() -> list[str]:
    ...
    if os.environ.get("LMSTUDIO_MODELS"):
        providers.append("lmstudio")
    if os.environ.get("LLAMASWAP_BASE_URL") and os.environ.get("LLAMASWAP_MODELS"):
        providers.append("llamaswap")
    if os.environ.get("CUSTOM_BASE_URL") and os.environ.get("CUSTOM_MODELS"):
        providers.append("custom")
    from .extra_providers import discover_extra_providers

    providers.extend(discover_extra_providers().keys())
    return providers
