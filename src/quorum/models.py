...
def _create_model_client_internal(model_id: str) -> ChatClient:
    ...
    elif settings.is_extra_provider(provider):
        cfg = settings.get_extra_provider_config(provider)
        if not cfg:
            raise ValueError(
                f"Extra provider '{provider}' not configured. "
                f"Set QUORUM_EXTRA_PROVIDERS and {provider.upper()}_* env vars in .env"
            )
        from .extra_providers import strip_extra_provider_prefix

        actual_model = strip_extra_provider_prefix(model_id)
        return OpenAIClient(
            model=actual_model,
            api_key=cfg.api_key or provider,
