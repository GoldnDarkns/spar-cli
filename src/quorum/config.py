...
class Settings(BaseSettings):
    ...
    # Additional OpenAI-compatible providers (groq, deepseek, together, …)
    # Example: QUORUM_EXTRA_PROVIDERS=groq,deepseek  +  GROQ_API_KEY, GROQ_MODELS, GROQ_BASE_URL
    quorum_extra_providers: str = Field(default="", alias="QUORUM_EXTRA_PROVIDERS")
    ...
    def available_providers(self) -> list[str]:
        ...
        providers.extend(self.extra_provider_keys())
        return providers

    def extra_provider_keys(self) -> list[str]:
        """Configured extra OpenAI-compatible provider keys (e.g. groq, deepseek)."""
        from .extra_providers import discover_extra_providers

        return list(discover_extra_providers(self.quorum_extra_providers).keys())

    def get_extra_provider_config(self, provider_key: str):
        """Return ExtraProviderConfig for a registered extra provider."""
        from .extra_providers import discover_extra_providers

        return discover_extra_providers(self.quorum_extra_providers).get(provider_key)

    def is_extra_provider(self, provider_key: str) -> bool:
        return provider_key in self.extra_provider_keys()
